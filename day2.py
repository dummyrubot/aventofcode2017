...
def proceed_row_sum(row):
    row_sum = 0
    for index, value in enumerate(row):
        if index == len(row) - 1:
            continue

        for elem in row[index + 1:]:
            temp_vals = [value, elem]
            division = max(temp_vals) / min(temp_vals)
...
def calculate_checksum(matrix):
    ...
    for row in matrix:
        checksum += proceed_row_sum(row)

    return checksum


if __name__ == '__main__':
    result = calculate_checksum(test_matrix)
    print(result)
